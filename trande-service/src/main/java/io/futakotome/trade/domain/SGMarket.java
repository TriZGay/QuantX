...
public class SGMarket implements RequestPlateInfo, RequestStockInfo {
    ...
    public void sendStockInfoRequest(PlateDtoMapper plateDtoMapper) {
        List<PlateDto> hkAllPlates = plateDtoMapper.searchByMarketEquals(QotCommon.QotMarket.QotMarket_SG_Security_VALUE);
        for (int i = 0; i < hkAllPlates.size(); i++) {
            ...
            LOGGER.info("SeqNo:" + seqNo + "新加坡市场请求股票信息:" + request.toString());
