...
@Component
@RocketMQMessageListener(consumerGroup = MessageCommon.RT_KL_MIN_5_CONSUMER_GROUP, topic = MessageCommon.RT_KL_MIN_5_TOPIC, consumeMode = ConsumeMode.ORDERLY)
public class RTKLMin5Listener extends AbstractKLineListener implements RocketMQListener<RTKLMessage> {
    ...
    public RTKLMin5Listener(RTKLMapper mapper, ConcurrentHashMap<String, WebSocketSender> senderMap) {
        this.mapper = mapper;
    ...
    public void onMessage(RTKLMessage rtklMessage) {
        ...
        if (mapper.insertOne(dto, RTKLMapper.KL_MIN_5_TABLE_NAME)) {
            LOGGER.info("5分K数据,[代码={},复权={},入库时间={}]入库成功", dto.getCode(), dto.getRehabType(), dto.getAddTime());
            sendKLineWsMessage(rtklMessage,
                    senderMap.get(AbstractWebSocketServerHandler.KLINE_MIN5_TAG));
        }
    }
