...
@Component
@WebSocketMapping("/websocket/kl_min15")
public class KLineMin15WsServerHandler
        extends AbstractWebSocketServerHandler implements WebSocketHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(KLineMin15WsServerHandler.class);
    private final ObjectMapper objectMapper;
    private final ConcurrentHashMap<String, WebSocketSender> senderMap;

    public KLineMin15WsServerHandler(ConcurrentHashMap<String, WebSocketSender> sendMap, ObjectMapper objectMapper) {
    ...
    public Mono<Void> handle(WebSocketSession session) {
        ...
        //todo 定点推送?
        String id = queryMap.getOrDefault("id", KLINE_MIN15_TAG);
//        Mono<Void> close = session.close()
//                .doOnNext(v -> {
//                    LOGGER.info("{}:连接关闭", KLINE_MIN15_TAG);
//                    senderMap.remove(id);
//                });
