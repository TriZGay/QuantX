...
//                sendNotifyMessage(notify);
    ...
    public void syncIndicatorList(IndicatorListWsMessage req) {
        ...
        if (Objects.nonNull(req.getLangType())) {
            c2sBuilder.setLangType(req.getLangType());
        }
        if (Objects.nonNull(req.getSearchKey())) {
            c2sBuilder.setSearchKey(req.getSearchKey());
        }
        if (Objects.nonNull(req.getSearchMode())) {
            c2sBuilder.setSearchMode(req.getSearchMode());
        }
    ...
    public void onReply_GetIndicatorList(FTAPI_Conn client, int nSerialNo, QotGetIndicatorList.Response rsp) {
        if (rsp.getRetType() != 0) {
            String notify = "查询指标列表失败:" + rsp.getRetMsg();
            LOGGER.error(notify, new IllegalArgumentException("请求序列号:" + nSerialNo + "查询指标列表失败,code:" + rsp.getRetType()));
            sendNotifyMessage(notify);
        } else {
            try {
                FTGrpcReturnResult ftGrpcReturnResult = GSON.fromJson(JsonFormat.printer().print(rsp), FTGrpcReturnResult.class);
                logFTResult("查询指标列表", ftGrpcReturnResult);
                IndicatorListContent content = GSON.fromJson(ftGrpcReturnResult.getS2c(), IndicatorListContent.class);
                eventPublisher.publishEvent(new IndicatorListUpdateEvent(content));
            } catch (InvalidProtocolBufferException e) {
                String errMsg = "查询指标列表结果失败.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (NullPointerException e) {
                String errMsg = "查询指标列表结果空指针.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (Exception e) {
                String errMsg = "查询指标列表有其他错误.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            }
        }
    ...
    public void syncEarningsBeatRank(EarningsBeatRankWsMessage req) {
        QotGetEarningsBeatRank.C2S.Builder c2sBuilder = QotGetEarningsBeatRank.C2S.newBuilder()
                .setMarket(req.getMarket())
                .setBeatType(req.getBeatType());
        if (Objects.nonNull(req.getCount())) {
            c2sBuilder.setCount(req.getCount());
        }
        if (Objects.nonNull(req.getTerm())) {
            c2sBuilder.setTerm(req.getTerm());
        }
        if (Objects.nonNull(req.getFilterList())) {
            List<QotGetEarningsBeatRank.Indicator> filterList = req.getFilterList()
                    .stream().map(f -> QotGetEarningsBeatRank.Indicator.newBuilder()
        ...
        QotGetEarningsBeatRank.Request request = QotGetEarningsBeatRank.Request.newBuilder()
                .setC2S(c2sBuilder.build()).build();
        int seqNo = qot.getEarningsBeatRank(request);
        LOGGER.info("市场={},查询盈利超预期排名.seq={}", MarketType.getName(req.getMarket()), seqNo);
    }

    @Override
    public void onReply_GetEarningsBeatRank(FTAPI_Conn client, int nSerialNo, QotGetEarningsBeatRank.Response rsp) {
        if (rsp.getRetType() != 0) {
            String notify = "查询盈利超预期排名失败:" + rsp.getRetMsg();
            LOGGER.error(notify, new IllegalArgumentException("请求序列号:" + nSerialNo + "查询盈利超预期排名失败,code:" + rsp.getRetType()));
            sendNotifyMessage(notify);
        } else {
            try {
                FTGrpcReturnResult ftGrpcReturnResult = GSON.fromJson(JsonFormat.printer().print(rsp), FTGrpcReturnResult.class);
                logFTResult("查询盈利超预期排名", ftGrpcReturnResult);
                EarningsBeatRankContent content = GSON.fromJson(ftGrpcReturnResult.getS2c(), EarningsBeatRankContent.class);
                eventPublisher.publishEvent(new EarningsBeatRankUpdateEvent(content));
            } catch (InvalidProtocolBufferException e) {
                String errMsg = "查询盈利超预期排名结果失败.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (NullPointerException e) {
                String errMsg = "查询盈利超预期排名结果空指针.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (Exception e) {
                String errMsg = "查询盈利超预期排名有其他错误.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            }
        }
    ...
    public void syncDividendCalendar(DividendCalendarWsMessage req) {
        QotGetDividendCalendar.C2S.Builder c2sBuilder = QotGetDividendCalendar.C2S.newBuilder()
                .setDate(req.getDate())
                .setMarket(req.getMarket());
        if (Objects.nonNull(req.getDataFrom())) {
            c2sBuilder.setDataFrom(req.getDataFrom());
        }
        if (Objects.nonNull(req.getCount())) {
            c2sBuilder.setCount(req.getCount());
        }
        QotGetDividendCalendar.Request request = QotGetDividendCalendar.Request.newBuilder()
                .setC2S(c2sBuilder.build()).build();
        int seqNo = qot.getDividendCalendar(request);
        LOGGER.info("市场={},查询派息日历.seq={}", MarketType.getName(req.getMarket()), seqNo);
    }

    @Override
    public void onReply_GetDividendCalendar(FTAPI_Conn client, int nSerialNo, QotGetDividendCalendar.Response rsp) {
        if (rsp.getRetType() != 0) {
            String notify = "查询派息日历失败:" + rsp.getRetMsg();
            LOGGER.error(notify, new IllegalArgumentException("请求序列号:" + nSerialNo + "查询派息日历失败,code:" + rsp.getRetType()));
            sendNotifyMessage(notify);
        } else {
            try {
                FTGrpcReturnResult ftGrpcReturnResult = GSON.fromJson(JsonFormat.printer().print(rsp), FTGrpcReturnResult.class);
                logFTResult("查询派息日历", ftGrpcReturnResult);
                DividendCalendarContent content = GSON.fromJson(ftGrpcReturnResult.getS2c(), DividendCalendarContent.class);
                eventPublisher.publishEvent(new DividendCalendarUpdateEvent(content));
            } catch (InvalidProtocolBufferException e) {
                String errMsg = "查询派息日历结果失败.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (NullPointerException e) {
                String errMsg = "查询派息日历结果空指针.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            } catch (Exception e) {
                String errMsg = "查询派息日历有其他错误.";
                LOGGER.error(errMsg, e);
                sendNotifyMessage(errMsg);
            }
        }
