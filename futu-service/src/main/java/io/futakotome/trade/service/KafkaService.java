...
public class KafkaService {
    ...
    public void onKLineUpdate(KLineUpdateEvent kLineUpdateEvent) {
        this.sendRTKLineMessage(kLineUpdateEvent.getContent());
    }

    public void sendRTKLineMessage(KLMessageContent klMessageContent) {
        if (!klMessageContent.getBlank()) {
            //内容不为空才发
            RTKLMessage message = toMessage(klMessageContent);
            send(klMessageContent.getKlType(), message);
        }
    }

    private RTKLMessage toMessage(KLMessageContent klMessageContent) {
    ...
    private void send(Integer rtKlType, RTKLMessage message) {
        if (rtKlType.equals(KLType.DAY.getCode())) {
            //日K
