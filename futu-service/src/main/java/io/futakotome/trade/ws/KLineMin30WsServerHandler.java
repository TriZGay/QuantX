...
@Component
@WebSocketMapping("/websocket/kl_min30")
public class KLineMin30WsServerHandler
        extends AbstractWebSocketServerHandler implements WebSocketHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(KLineMin30WsServerHandler.class);
    private final ObjectMapper objectMapper;
    private final ConcurrentHashMap<String, WebSocketSender> senderMap;

    public KLineMin30WsServerHandler(ObjectMapper objectMapper, ConcurrentHashMap<String, WebSocketSender> senderMap) {
    ...
    public Mono<Void> handle(WebSocketSession session) {
        ...
        //todo 定点推送?
        String id = queryMap.getOrDefault("id", KLINE_MIN30_TAG);
//        Mono<Void> close = session.close()
//                .doOnNext(v -> {
//                    LOGGER.info("{}:连接关闭", KLINE_MIN30_TAG);
//                    senderMap.remove(id);
//                });
