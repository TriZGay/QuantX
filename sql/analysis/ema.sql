...
with recursive raw_data as (
    ...
    select market, code, rehab_type, close_price, ema_5, ema_10, ema_12, ema_20, ema_26, ema_60, ema_120, update_time, rn
    from pre_data
    where rn = 1
    union all
    select p.market, p.code, p.rehab_type, p.close_price, (p.close_price*2+e.ema_5 * 4)/6 as ema_5, (p.close_price*2+e.ema_5 * 9)/11 as ema_10, (p.close_price*2+e.ema_5 * 11)/13 as ema_12, (p.close_price*2+e.ema_5 * 19)/21 as ema_20, (p.close_price*2+e.ema_5 * 25)/27 as ema_26, (p.close_price*2+e.ema_5 * 59)/61 as ema_60, (p.close_price*2+e.ema_5 * 119)/121 as ema_120, p.update_time, p.rn
...
from t_ema_min_1_arc e
         inner join (select code, rehab_type, max(update_time) as update_time
